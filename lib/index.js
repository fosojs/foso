...
Foso.prototype.now = function(cb) {
  ...
  if (this._opts.serve) {
    var serverOpts = R.type(this._opts.serve) === 'Object' ?
      this._opts.serve : this._getDefaultServe();
    server(this._opts.dest, serverOpts);
    this._opts.host = this._opts.host || 'localhost:' + serverOpts.port;
    if (serverOpts.securePort) {
      this._opts.secureHost = this._opts.secureHost ||
  ...
  async.applyEachSeries(this._plugins, this._opts, function(err) {
    if (this._opts.livereload) {
      var extensions = R.union(R.map(function(plugin) {
