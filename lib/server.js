...
function serve(basePath, options) {
  options = options || {};
  ...
  var app = connect();
  app.use(serveStatic(options.buildFolder));
  ...
  https.createServer(serverOptions, app).listen('1770');
  ...
  var watcher = watch(path.join(options.buildFolder, '*.js'));
